...
void showMessage(GtkWidget *parent, GtkMessageType type, char *mms,
    ...
    mdialog = gtk_message_dialog_new(GTK_WINDOW(parent),
                                     ...
                                     GTK_BUTTONS_OK, "%s", mms);
    gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(mdialog), "%s",
                                             content);
...
void showCreateOrJoinDialog() {
    if (createOrJoinDialog == NULL) initCreateOrJoinDialog();
...
void initCreateOrJoinDialog() {
    createOrJoinDialog = gtk_dialog_new_with_buttons(
        APP_TITLE, GTK_WINDOW(window),
    ...
    gtk_box_pack_start(
        GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(createOrJoinDialog))),
        dialog_ground, TRUE, TRUE, 0);

    GtkWidget *data_array[3];
    data_array[0] = inputUsername;
    data_array[1] = inputRoomName;
    data_array[2] = createOrJoinDialog;
    g_signal_connect(createButton, "clicked", G_CALLBACK(onCreateButtonClicked),
                     data_array);
    g_signal_connect(joinButton, "clicked", G_CALLBACK(onJoinButtonClicked),
                     data_array);
...
GtkWidget *initMessageInput(int x, int y) {
    ...
    g_signal_connect(sendButton, "clicked", G_CALLBACK(onSendButtonClicked),
                     NULL);
    return messageInput;
}
...
void textViewSetText(GtkWidget *textView, char *text) {
    ...
    GtkTextTag *tag =
        gtk_text_buffer_create_tag(t_buffer, NULL, "foreground", "blue",
    ...
    GtkAdjustment *adj = gtk_scrolled_window_get_vadjustment(
        GTK_SCROLLED_WINDOW(chatOutputScroller));
    gtk_adjustment_set_value(adj, gtk_adjustment_get_upper(adj));
    gtk_scrolled_window_set_vadjustment(GTK_SCROLLED_WINDOW(chatOutputScroller),
                                        adj);
}
...
void initCurrentUserBox(int x, int y) {
    ...
    g_signal_connect(exitRoomButton, "clicked",
                     G_CALLBACK(onExitRoomButtonClicked), NULL);
}
...
GtkWidget *initPublicChannelBox(int x, int y) {
    ...
    gtk_container_add(GTK_CONTAINER(publicChannelGroupBox),
                      publicChannelButton);

    g_signal_connect(publicChannelButton, "clicked",
                     G_CALLBACK(onChannelButtonClicked), PUBLIC);
...
int setButtonFocus(GtkWidget *button, char *s) {
    ...
    gtk_widget_override_background_color(GTK_WIDGET(button), GTK_STATE_NORMAL,
                                         &color);
    return 0;
}
...
void addButtonToUserListBox(char n[][32], int count) {
    ...
    for (i = 0; i < count; ++i) {
        if (strcmp(n[i], you) != 0) {
            ...
            g_signal_connect(userIndex, "clicked",
                             G_CALLBACK(onChannelButtonClicked), n[i]);
        }
    }
