...
void clearBuffer(char *buff) { memset(buff, 0, strlen(buff)); }
...
char *saveToUserMessageStream(char *sender, char *message) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++) {
        if (strcmp(onlineUsersStream[i].username, sender) == 0) {
            ...
            onlineUsersStream[i].stream =
                (char *)calloc(69 * MAXLINE, sizeof(char));
            found = 1;
        }
        if (found) {
            sprintf(temp, "%s\n", message);
...
int createClient(int argc, char *argv[]) {
    ...
    if (connect(clientSocket, (struct sockaddr *)&server_socket,
