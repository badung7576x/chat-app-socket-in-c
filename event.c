...
void onCreateRoomSuccess(char *message) {
	...
	runInUIThread(showMessage(createOrJoinDialog, GTK_MESSAGE_WARNING, CREATE_ROOM_SUCCESS, WELLCOME));
...
void onCreateOrJoinFailed(char *message){
	runInUIThread(showMessage(createOrJoinDialog, GTK_MESSAGE_ERROR, CREATE_ROOM_FAILED, message));
}

void onCreateButtonClicked(GtkWidget *widget, gpointer gp) {
	strcpy(username, (char *)gtk_entry_get_text(GTK_ENTRY(inputUsername)));
	strcpy(roomName, (char *)gtk_entry_get_text(GTK_ENTRY(inputRoomName)));
...
void onSendButtonClicked(GtkWidget *widget, gpointer data){
	...
	if (strcmp(currentChannel, PUBLIC) == 0)
		...
		sprintf(inputBuffer, "%c%s#%s", PRIVATE_MESSAGE_ACTION, currentChannel, text);
	}
	sendRequest();
}